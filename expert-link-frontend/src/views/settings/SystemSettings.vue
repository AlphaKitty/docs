        ...
        <el-tab-pane label="权限设置" name="permissions">
          <el-card class="settings-card">
            ...
            <template v-if="auth.isSuperAdmin">
              <el-divider />
              <h4>菜单与路由角色可见性策略</h4>
              <p class="form-help">该配置会实时影响侧栏菜单显示、路由访问与申请详情页按钮显隐。</p>
              <el-form label-width="180px">
                <el-form-item label="行管能力角色组">
                  <el-checkbox-group v-model="systemSettings.roleGroups.STEWARD">
                    <el-checkbox v-for="r in roleOptions" :key="`st-${r.value}`" :label="r.value">
                      {{ r.label }}
                    </el-checkbox>
                  </el-checkbox-group>
                </el-form-item>
                <el-form-item label="专家能力角色组">
                  <el-checkbox-group v-model="systemSettings.roleGroups.EXPERT">
                    <el-checkbox v-for="r in roleOptions" :key="`ex-${r.value}`" :label="r.value">
                      {{ r.label }}
                    </el-checkbox>
                  </el-checkbox-group>
                </el-form-item>
                <el-form-item label="超管能力角色组">
                  <el-checkbox-group v-model="systemSettings.roleGroups.SUPER_ADMIN">
                    <el-checkbox v-for="r in roleOptions" :key="`sa-${r.value}`" :label="r.value">
                      {{ r.label }}
                    </el-checkbox>
                  </el-checkbox-group>
                </el-form-item>
                <el-form-item>
                  <el-button type="primary" @click="saveRoleGroupSettings">保存角色策略</el-button>
                  <el-button @click="resetRoleGroupSettings">恢复默认</el-button>
                </el-form-item>
              </el-form>
            </template>
          </el-card>
...
void systemSettings.ensureHydrated()
const roleOptions: { label: string; value: AppRole }[] = [
  { label: '超级管理员', value: 'SUPER_ADMIN' },
  { label: '领域行管', value: 'DOMAIN_STEWARD' },
  { label: '部门管理员', value: 'DEPT_ADMIN' },
  { label: '专家用户', value: 'EXPERT_USER' },
  { label: '普通用户', value: 'REGULAR_USER' },
]
...
const saveRoleGroupSettings = async () => {
  try {
    await systemSettings.saveRoleGroups()
    ElMessage.success('角色策略已保存')
  } catch (e: unknown) {
    ElMessage.error((e as Error)?.message || '保存失败')
  }
}

const resetRoleGroupSettings = async () => {
  try {
    await systemSettings.resetRoleGroups()
    ElMessage.success('角色可见性策略已重置为默认值')
  } catch (e: unknown) {
