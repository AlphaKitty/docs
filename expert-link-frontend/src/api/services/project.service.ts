...
function toProjectPayload(data: CreateProjectRequest | UpdateProjectRequest) {
  return {
    name: data.name,
...
function toProjectParams(data: CreateProjectRequest | UpdateProjectRequest) {
  return {
    domainId: data.domainId,
    createdById: 'createdById' in data ? data.createdById ?? data.managerId : undefined,
    lastUpdatedById: data.managerId,
  };
}
...
export class ProjectService {
  ...
  static async exportProjects(params?: ProjectQueryParams): Promise<Blob> {
    try {
      return await apiClient.get<Blob>('/projects/export', {
        params,
        responseType: 'blob',
      });
    } catch (error) {
